...
**Repeated-failure hint:** when the transcript shows the same tool called again with identical arguments and an identical error result, the proxy appends a strategy-change note to the next turn so the model stops re-running a call that will fail the same way.
